...
object StopWords {
    ...
    def removeStopwords( sl:List[String]) :List[String] = 
      sl filter ((x)=> !(stopList.exists(_ == x)))
    
    /*
    def removeStopwordsFromFile( infile:String) =
      infile match {
        case "" => println("No filename given for filtering")
        case _ => { val filteredLists = for( line <- (Source.fromFile(new File(infile))).getLines) 
                    yield {
                      removeStopwords(List.fromString(line,' '))
                      }
                    print( filteredLists.reduceLeft(_ ::: _) )
                    }
      }
    
    def main(args : Array[String]) : Unit = {
      if (args.length > 0) {
        removeStopwordsFromFile(args(0))
      }
    }
    */
}
